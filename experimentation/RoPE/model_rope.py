...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        #cos and sin values calculation
...
class GPT(nn.Module):
    ...
    def get_num_params(self, non_embedding=True):
        ...
        # if non_embedding: 
        #     n_params -= self.transformer.wpe.weight.numel() removed this
        return n_params
    ...
    def forward(self, idx, targets=None):
        ...
        # pos = torch.arange(0, t, dtype=torch.long, device=device) # shape (t) commented for rope

        # forward the GPT model itself
        tok_emb = self.transformer.wte(idx) # token embeddings of shape (b, t, n_embd) => (b,t)=> (b,t,n_embd)
        # pos_emb = self.transformer.wpe(pos) # position embeddings of shape (t, n_embd) commented for rope
